...
function eq_tbl.forget() return eq_forget() end
...
local EQ_ROUTES = {
   scan = function(arg) return eq_tbl.scan(arg ~= "" and arg or nil) end,
   quick = function(_) return eq_tbl.quick() end,
   compare = function(arg) return eq_tbl.compare(arg) end,
   cmp = function(arg) return eq_tbl.compare(arg) end,
   id = function(arg) return eq_tbl.id(arg) end,
   identify = function(arg) return eq_tbl.id(arg) end,
   shop = function(_) return eq_tbl.shop() end,
   stats = function(_) return eq_tbl.stats() end,
   cache = function(_) return eq_tbl.stats() end,
   forget = function(_) return eq_tbl.forget() end,
}
...
setmetatable(eq_tbl, { __call = function(_, rest)
   ...
   local handler = EQ_ROUTES[verb]

   if handler then return handler(arg) end
   echo(eq_usage(), "cyan")
end, })
...
id_begin = function() S.id_cap = { lines = {} } end
id_feed = function(line)
   local cap = S.id_cap
   if not cap then return end
   local L = strip_ansi(line)

...
local opps
local opponent_note
local parse_opponent
local parse_melee
local melee_enemy
local is_self
local maybe_assist
local reset_assist
local parse_target_line
...
opps = function() return state.opponents end
...
opponent_note = function(tbl, name, pct, now, exact)
...
reset_assist = function() state.assisted = nil end



is_self = function(name)
   if not name then return false end
   local low = name:lower()
...
melee_enemy = function(attacker, target)
...
parse_target_line = function(line)
   local t = line or ""

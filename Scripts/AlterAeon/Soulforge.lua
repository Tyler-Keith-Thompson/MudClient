...
local loop_if = __machine.loop_if
...
local sf_fsm = (machine("gather")):
phase("gather", function(advance) start_gather(advance) end):
phase("forge", function(advance) plan_and_cast(advance) end):
phase("stow", function(advance) begin_stow(advance) end,
loop_if(function() return made_progress() end))
...
local function resync_and_plan()
   ...
   if not op.gathered then sf_fsm:start(function() report_done(); sf_settle(true) end); return end
   plan_and_cast()
end
...
plan_and_cast = function(advance)
   local op = sf_op; if not op then return end
   if advance then op.advance = advance end
...
start_gather = function(advance)
   local op = sf_op; if not op then return end
   op.advance = advance
   op.forged_this_cycle = 0
   if op.gathered then return done_looking() end
   ...
   if #op.containers == 0 then op.gathered = true; return op.advance() end
...
forge_done = function()
   local op = sf_op; if not op then return end
   if cfg.stow and op.stow_kw and #(op.model or {}) > 0 then return op.advance() end
   op.advance(machine.DONE)
end
...
made_progress = function()
   local op = sf_op
   return op ~= nil and (op.forged_this_cycle or 0) > 0 and container_has_raw(op)
end
...
local function begin_op()
   ...
   local p = __promise(function(resolve, reject, onCancel)
      sf_op = { resolve = resolve, reject = reject, cmd = nil, no_progress = 0, gathered = false }
      onCancel(function()
...
local function hit_get_ok(color_phrase)
   local op = sf_op; if not op or sf_fsm:current() ~= "gather" or op.gphase ~= "pull" then return end
...
local function hit_carry_full()
   local op = sf_op; if not op or sf_fsm:current() ~= "gather" then return end
...
local function hit_put_ok(color_phrase)
   local op = sf_op; if not op or sf_fsm:current() ~= "stow" then return end

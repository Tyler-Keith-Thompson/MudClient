...
final class ServerTextFeed: @unchecked Sendable {
    ...
    /// Inject `data` — ONE COMPLETE server message (a decoded protobuf `text_block`) — into the inbound
    /// pipeline. Starts the pump lazily. The bytes are rendered exactly as received; nothing is appended.
